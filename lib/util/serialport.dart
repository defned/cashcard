...
abstract class MySerialPortReader {
  ...
  /// to 500ms.
  factory MySerialPortReader(SerialPort port, {int? timeout}) =>
      _MySerialPortReaderImpl(port, timeout: timeout);
...
class _MySerialPortReaderImpl implements MySerialPortReader {
  ...
  _MySerialPortReaderImpl(SerialPort port, {int? timeout})
      : _port = port,
  ...
  static final List<int> readBytes = [];
  static void _waitRead(_SerialPortReaderArgs args) {
    ...
    while (bytesWaiting >= 0) {
      dylib.sp_flush(port, sp_buffer.SP_BUF_BOTH);
      bytesWaiting = _waitEvents(port, events, 250);
      if (bytesWaiting > 0) {
        final data = Util.read(bufMaxLen, (ffi.Pointer<ffi.Uint8> ptr) {
          return dylib.sp_nonblocking_read(port, ptr.cast(), bufMaxLen);
        });
        readBytes.addAll(data);
